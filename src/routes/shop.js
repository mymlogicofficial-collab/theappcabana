  ...
  try {
    ...
    let query = `
      SELECT p.*, u.display_name as creator,
        COALESCE((SELECT AVG(rating) FROM reviews WHERE reviews.product_id = p.id), 0)::NUMERIC as avg_rating,
        COALESCE((SELECT COUNT(*) FROM reviews WHERE reviews.product_id = p.id), 0)::INT as review_count
      FROM products p
